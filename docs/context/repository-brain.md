...
| **Virtual Desktop Pinning Rules** | `castervoice/lib/windows_virtual_desktops.py` & `window_mgmt_rule.py` | [`docs/pyvda/003`](../pyvda/003_pyvda_multi_window_xaml_island_pinning_architecture.md) | Production / Branch `feat/virtual-desktop-pinning` |
...
- **PyVDA COM Lifecycle & Multi-Window Pinning:** Undocumented Windows Virtual Desktop COM interfaces hosted in `explorer.exe` can become invalid if Explorer restarts. `pyvda`'s `@_com_retry` decorator with exponential backoff and GUID re-hydration (`_refresh()`) is the verified pattern for preventing `RPC_S_SERVER_UNAVAILABLE` (`0x800706BA`) and `RPC_E_DISCONNECTED` (`0x80010108`) crashes. Furthermore, Windows Virtual Desktop Manager evaluates pinned AppIDs via strict exact string matching (`wcscmp`). In modern packaged and XAML Island applications (e.g. Windows Terminal, Antigravity IDE), secondary windows are assigned synthetic sub-AUMIDs (`<Package>!App~Wh~w<HEX_HWND>`). `pyvda` decouples canonical application identity (`base_app_id`), pins active sub-views via in-memory `PinView` (preventing transient registry pollution), and synchronizes newly opened windows across workspace transitions (`sync_pinned_apps`).
