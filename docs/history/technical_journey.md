...
- **Status (Active Exploration & Prototyping - Verified)**: Successfully tested and verified that voice grammars dynamically activate based on element-level desktop interaction zones—specifically activating terminal rules (`IDETerminalRule`) within the integrated terminal instance of VS Code / Antigravity IDE using the Active Desktop Context Engine (ADCE) and Dragonfly `FuncContext`.
...
- **Key Documentation**:
  * 📘 **[ADCE Dynamic IDE Terminal Context Guide](../features/adce_dynamic_terminal_context_guide.md)** *(Feature Guide & Runbook)*
