...
2. **The True Issue: In-Process COM Apartment Deadlocks**: The true root cause of engine freezes was not synchronous blocking itself, but executing Python COM wrappers (`pywinauto`, `comtypes`) *in-process* on Dragonfly's main engine thread without a dedicated Win32 message pump. In-process COM calls in Python routinely deadlock or hang when Windows sessions lock, minimize, or switch window stations.
...
- [Ticket 006: Research Caster's Current UIA Usage](tickets/006_caster_uia_research.md) ([Breakdown](research/006_caster_uia_usage_educational_breakdown.md)) — Evaluates Caster's synchronous pywinauto calls on the main voice thread, highlighting the need to offload UI operations.
