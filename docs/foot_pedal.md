...
`foot_pedal.ahk` provides advanced control for the Olympus RS31H USB foot pedal, paired with a background Python XML-RPC server (`caster_toggle_mic_key.py`) for thread-safe, hands-free Caster microphone toggling.
...
### 2. AutoHotkey v2 Client Integration (`foot_pedal.ahk`)
...
### Python XML-RPC Server (`caster_toggle_mic_key.py`)
