...
class CustomPython(MappingRule):
    mapping = {
        "with": R(Text("with ")),
        "open file": R(Text("open('filename','r') as f:")),
        "read lines": R(Text("content = f.readlines()")),
        "try catch": R(Text("try:") + Key("enter:2/10, backspace") + Text("except Exception:") + Key("enter")),
        # pdb
        "set trace": Text("pdb.set_trace()"),
        "break point": Text("breakpoint()"),
        "random list [<length>] [<element_magnitude>]": Function(_random_number_list),
        "a var": R(Text("a = ")),
        "b var": R(Text("b = ")),
        "c var": R(Text("c = ")),
    }
