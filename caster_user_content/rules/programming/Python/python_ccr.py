...
class CustomPythonCCR(MergeRule):
    ...
    mapping = {
        SymbolSpecs.IF: R(Key("i,f,space,colon,left")),
        SymbolSpecs.ELSE: R(Text("else:") + Key("enter")),
        # (no switch in Python)
        SymbolSpecs.BREAK: R(Text("break")),
        SymbolSpecs.FOR_EACH_LOOP: R(Text("for  in :") + Key("left:5")),
        SymbolSpecs.FOR_LOOP: R(Text("for i in range(0, ):") + Key("left:2")),
        SymbolSpecs.WHILE_LOOP: R(Text("while :") + Key("left")),
        # (no do-while in Python)
        SymbolSpecs.TO_INTEGER: R(Text("int()") + Key("left")),
        SymbolSpecs.TO_FLOAT: R(Text("float()") + Key("left")),
        SymbolSpecs.TO_STRING: R(Text("str()") + Key("left")),
        SymbolSpecs.AND: R(Text(" and ")),
        SymbolSpecs.OR: R(Text(" or ")),
        SymbolSpecs.NOT: R(Text("!")),
        SymbolSpecs.SYSOUT:
        # R(Store() + Text("print()") + Key("left") + Retrieve(action_if_text="right")),
        R(Text("print()") + Key("left")),
        SymbolSpecs.IMPORT: R(Text("import ")),
        SymbolSpecs.FUNCTION: R(Text("def ():") + Key("left:3")),
        SymbolSpecs.CLASS: R(Text("class :") + Key("left")),
        SymbolSpecs.COMMENT: R(Text("#") + Key("space")),
        SymbolSpecs.LONG_COMMENT: R(Text("''''''") + Key("left:3")),
        SymbolSpecs.NULL: R(Text("None")),
        SymbolSpecs.RETURN: R(Text("return ")),
        SymbolSpecs.TRUE: R(Text("True")),
        SymbolSpecs.FALSE: R(Text("False")),
        # Python specific
        "sue iffae": R(Text("if ")),
        "sue shells": R(Text("else ")),
        "from": R(Text("from ")),
        "self": R(Text("self")),
        "long not": R(Text(" not ")),
        "it are in": R(Text(" in ")),
        "shell iffae | LFA": R(Key("e,l,i,f,space,colon,left")),
        "convert to character": R(Text("chr()") + Key("left")),
        "length of": R(Text("len()") + Key("left")),
        "global": R(Text("global ")),
        "make assertion": R(Text("assert ")),
        "list (comprehension | comp)": R(Text("[x for x in TOKEN if TOKEN]")),
        "dot (pie | pi)": R(Text(".py")),
        "toml": R(Text("toml")),
        "jason": R(Text("toml")),
        "identity is": R(Text(" is ")),
        "yield": R(Text("yield ")),
