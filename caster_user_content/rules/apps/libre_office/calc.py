...
class CalcRule(MappingRule):
    mapping = {
        ...
        "fly <column_1> <row_1>": R(Function(_select_cell)),
        "select <column_1> <row_1> through <column_2> <row_2>": R(
            Key("c-g") + Text("%(column_1)s%(row_1)s:%(column_2)s%(row_2)s") + Key("enter")
        ),
...
def get_rule():
    return CalcRule, RuleDetails(name="Calc Rule", title="LibreOffice Calc")
