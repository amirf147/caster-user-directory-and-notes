...
class PowerPointRule(MappingRule):
    mapping = {
        "switch focus [<n>]": R(Key("f6:%(n)d")),
        "shin focus [<n>]": R(Key("s-f6:%(n)d")),
        "zoom in [<n>]": R(Key("a-w/3, q/3, tab, up:%(n)d, enter")),
        "zoom out [<n>]": R(Key("a-w/3, q/3, tab, down:%(n)d, enter")),
        "file custom retain": R(Key("a-f/3, a")),
        "cycle case": R(Key("s-f3")),
        "hint <ribbon>": R(Key("a-%(ribbon)s")),
        # Text
        "center align": R(Key("c-e")),
        "lay align": R(Key("c-l")),
        "indent": R(Key("as-right")),
        "unindent": R(Key("as-left")),
        # Font
        "font color": R(Key("a-h, f, c")),
        "font size": R(Key("a-h, f, s")),
        "text increase [<n10>]": R(Key("cs->:%(n10)d")),
        "text decrease [<n10>]": R(Key("cs-<:%(n10)d")),
        ...
        # Table
        "merge cells": R(Key("a-j, l, m")),
        # Slides
        "slide <n0_50>": R(Function(_change_slide)),
        "new slide": R(Key("a-h/3, z, s, i")),
        "duplicate slide": R(Key("c-d")),
        "slide layout": R(Key("a-h, l")),
        # Animations
        "add animation": R(Key("a-a/3, a, a")),
        "(annie | animation) start": R(Key("a-a/3, t")),
        "<pane> pane": R(Key("%(pane)s")),
        "insert <insert_action>": R(Key("%(insert_action)s")),
        "show grid lines": R(Key("s-f9")),
