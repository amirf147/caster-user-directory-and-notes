...
class FileDialogRule(MappingRule):
    ...
    mapping = {
        ...
        "copy address": R(Key("a-d/5, c-c")),
        "folder new": R(Key("cs-n")),
        "file new": R(Key("a-f, w, t")),
        "(show | file | folder) properties": R(Key("a-enter")),
        "dirrup [<n>]": R(Key("a-up:%(n)d")),
        "go back [<n>]": R(Key("a-left:%(n)d")),
        "go forward [<n>]": R(Key("a-right:%(n)d")),
        "search [<text>]": R(Key("a-d, tab:1") + Text("%(text)s", pause=0.0)),
        "(navigation | nav | left) pane": R(Key("a-d, tab:4")),
        "(center pane | (file | folder) (pane | list))": R(Key("a-d, tab:5")),
        ...
        "(go | navigate) <path>": R(Key("a-d/5") + Text("%(path)s", pause=0.0) + Key("enter, tab:6")),
        "go clipboard": R(Key("a-d/5, c-v, enter")),
    }
    extras = [
