...
class CustomMouseAlternativesRule(MappingRule):
    mapping = {
        "legion [<monitor>] [<rough>]": R(Function(Grid.mouse_alternates, mode="legion")),
        # "rainbow [<monitor>]":
        #     R(Function(Grid.mouse_alternates, mode="rainbow")),
        # "douglas [<monitor>]":
        #     R(Function(Grid.mouse_alternates, mode="douglas")),
        "sudoku [<monitor>]": R(Function(Grid.mouse_alternates, mode="sudoku")),
    }
    extras = [ShortIntegerRef("monitor", 1, 10), Choice("rough", {"rough": True, "detailed": False})]
    defaults = {"rough": True}
