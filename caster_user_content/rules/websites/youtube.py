...
class YoutubeRule(MappingRule):
    ...
    mapping = {
        "check comments": R(Key("escape/200, c-end/200, pgdown")),
        # Only works when on a page that is not a youtube video
        "search <query>": R(Key("f1/2") + Text("%(query)s")),
        # TODO: Not working, fix this when I have time
        "speed up [<n>]": R(Key("escape/8, escape") + Pause("100") + Key("i/3") + Key("colon:%(n)d")),
        "slow down [<n>]": R(Key("escape/8, escape") + Pause("100") + Key("i/3") + Key("semicolon:%(n)d")),
    }
    extras = [
